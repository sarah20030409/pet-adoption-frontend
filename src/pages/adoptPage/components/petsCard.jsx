        ...
        <>
          {Pet.map((Pet) => (
            <PetsCard
              ...
              age={Pet.age}
              variety={Pet.variety}
              petImage={`${process.env.REACT_APP_API_BASE_URL}/${Pet.petImage}`}
...
export function PetsCard({ id, name, age, variety, petImage, isDisabled }) {
  ...
  const handleClick = () => {
    axios
      .post(`${process.env.REACT_APP_API_BASE_URL}/adoptedPet`, { id, userId })
      .then((response) => {
  ...
  async function GetNameById(pId) {
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_API_BASE_URL}/owner/${pId}`
      );
      ...
      console.error(
        "There was an error fetching the pet card's owner name!",
        error
      );
    }
  }
    ...
    <div className="petsCard">
      <div className="cardBackground">
        ...
        <div className="btnMiddle">
          <button
            className={isClicked ? "adoptBtn clicked" : "adoptBtn"}
            onClick={handleClick}
            disabled={isClicked}
          >
            ADOPT
          </button>
          <p className="more" onClick={goMore}>
            More →
          </p>
        </div>

        <p className={noOwner ? "Owner NoOwner" : "Owner"}>
